...
historical date, so we never request OHLCV for a symbol that was not yet
trading (or had already been delisted).
...
Rate-limit / retry
------------------
A sliding-window token bucket caps outbound traffic at ~100 requests per
60 seconds (the Polygon Starter tier ceiling). Each transient failure
(HTTP 429 or 5xx, or a low-level network error) triggers exponential
backoff with three attempts (1s, 2s, 4s plus jitter). The bucket is
process-local; cross-process throttling would need an external store.
"""
...
class PolygonProvider:
    ...
    api_key:
        ...
        environment variable. A missing key raises :class:`PolygonAuthError`
        immediately so misconfiguration surfaces at construction time rather
        than on the first network call.
    session:
        Optional pre-built :class:`httpx.Client`. When ``None`` the provider
        owns the lifecycle and closes the session in :meth:`close`.
    rpm:
        Requests-per-minute ceiling for the token bucket. Defaults to the
        Polygon Starter tier limit of 100.
    """
